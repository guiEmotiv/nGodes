...
func GetNews(v []NewFormatTasks, guard int) (test []GetPlan, PA, PA1 float64) {
	...
	var weightByScore = make([]float64,lenAllTaskIDs)
				...
				if mapAllIdTask != nil {
					...
					if mapAllIdTask[a].NewTaskType != 0 {
						...
						if deleteId == selectScore {
							...
							weightByScore[deleteId] = sliceAllIdTask[selectScore].NewImportance
							fmt.Printf("TASK ACCOMPLISHED (Delete ID # %d) \n", deleteId)
						}else {
							fmt.Println("--- WARNING UNFULFILLED TASK")
						}

						fmt.Println("LIST SCORES ACCOMPLISHED By ID (Best Weight): ", sumScore)
						fmt.Println("LIST SCORES ACCOMPLISHED By WEIGHT: ", weightByScore)
						fmt.Printf("\n")
						...
						t1 := scoreGlobal(weightByScore)

						// SAME FOR LOOP BUT CONDITION ALARM TIME
						if stepTime + 1 == float64(int64(StoreEmergency[bestEmergency[0]].NewEarliest)) {
							...
							for i := 0; i < len(e); i++ {
								...
								updateY = &e[i].LocY
								fmt.Println(StorePlan)
								//e[i].LocX == StoreEmergency[bestEmergency[0]].LocX && e[i].LocY == StoreEmergency[bestEmergency[0]].LocY
								if e[i].Dist == 0 && e[i].Duration == StoreEmergency[bestEmergency[0]].NewDuration && e[i].TypeStatus == 2 && e[i].LocX == StoreEmergency[bestEmergency[0]].LocX && e[i].LocY == StoreEmergency[bestEmergency[0]].LocY{
									deleteIdEmergency =  bestEmergency[0]
								}

							}
							delete(StoreEmergency,deleteIdEmergency)
							sumScore[idPlanTaskEmer[bestEmergency[0]]] = 100
							weightByScore[idPlanTaskEmer[bestEmergency[0]]] = 100
						...
						fmt.Printf("\n")
						fmt.Println("ACTUALIZACION SCORE By BEST WEIGHT------ : ",t)
						fmt.Println("ACTUALIZACION SCORE By WEIGHT------ : ",t1)
						fmt.Printf("\n")
						PA = t
						PA1 = t1
					}else {
...
func nextPosition(x1, y1 float64 , et float64) (bestPos int) {
	...
	fmt.Println("que chucha: ", selectScore)
...
func runOrdinaryTask(x1, y1 float64, x2, y2 float64, et float64, a int) (newPos []StepPos) {
	...
	for i := 0.0; i <= arrDist; i++ {
		...
		//fmt.Println("entre")
		MC0, MC1, MC2, MC3 := matrixCoverage(newX2, newY2, idOrd, coverageRadius)
		//fmt.Println("sali")
	...
	if et < mapAllIdTask[a].NewEarliest {
		...
		for i := 0.0; i< waitingTime; i++ {
			...
			MC0, MC1, MC2, MC3 := matrixCoverage(lastPostStoreXWaiting, lastPostStoreYWaiting, lastElapsedTimeWaiting, coverageRadius)
			StorePos["travelOrd"] = StepPos{
				idTask,
				typeWaitingOrd,
				i,
	...
	for i := 0.0; i <= operationTime; i++ {
		...
		//fmt.Println("wait",i)
		MC0, MC1, MC2, MC3 := matrixCoverage(lastPostStoreX, lastPostStoreY, lastElapsedTime, coverageRadius)
		lastElapsedTime++
		StorePos["travelOrd"] = StepPos{
			idTask,
			typeOperationOrd,
...
func runEmergencyTask(x1, y1, x2, y2 float64, et float64) (newPos []StepPos) {
	...
	idTask := 2
	typeOperationEmergency := 2
	typeTravelEmergency := 1
	...
	for i := 0.0; i <= arrDist; i++ {
		...
		MC0, MC1, MC2, MC3 := matrixCoverage(newX2, newY2, idEmergency, coverageRadius)
	...
	lastPostStoreY := y2
	lastElapsedTime := StorePos["travelOrd"].TimeElapsed
	operationTime := StoreEmergency[bestEmergency[0]].NewDuration// 7.5
...
func matrixCoverage(x2, y2 float64, et float64, cov float64) (covFeasibleIDs, sumCovFeaIDs float64, covDetails, covDetailsMatrix string) {
	...
	accumulatedCoverageTime = 0
	accumulatedCoverageSite = 0
	...
	for k, v := range sliceMatrixCoverage {
		...
		probNoAlarm = math.Exp(-meanRateAlarmsperShift * (0.0001))
	...
	sumCovFeaIDs = accumulatedCoverageSite

	var res float64
	for k, v := range sliceMatrix {

		if v == 0 { continue }
		if k == 0 { continue }
		accumulatedCoverageTime += v

	}
	//fmt.Println("vivhvjv",accumulatedCoverageTime)
	res = (float64(accumulatedCoverageTime) / 12.0)
	covFeasibleIDs = res
	//fmt.Println("fafaf",res)

	//fmt.Println(covFeasibleIDs)
	//fmt.Println("CTM AQUI ESTOY ",sliceMatrixFeasible)
